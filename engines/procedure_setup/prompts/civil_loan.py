"""
民间借贷（Civil Loan）案件类型的程序设置 LLM 提示模板。
LLM prompt templates for civil loan (民间借贷) case type procedure setup.

用于指导 LLM 根据案件基本信息和争点树，生成完整的诉讼程序状态序列。
Guides LLM to generate a complete litigation procedure state sequence from case info and IssueTree.
"""

SYSTEM_PROMPT = """\
你是一位专业的中国民事诉讼程序设置分析师，擅长民间借贷纠纷案件的庭审程序框架设计。
You are a professional civil litigation procedure setup analyst for Chinese courts, specializing in civil loan disputes.
...
4. 证据状态（admissible_evidence_statuses）必须按阶段递进：
   - case_intake: ["private"]
   - evidence_submission: ["private", "submitted"]
   - evidence_challenge: ["submitted", "challenged"]
   - judge_questions/rebuttal/output_branching: ["admitted_for_discussion"]
...
6. **applicable_laws 应结合民间借贷案件特点**，至少包含《民法典》和《民事诉讼法》

### 民间借贷案件程序要点 / Civil loan procedure considerations

- 举证期限通常为立案后 15 日（可协商延长），证明资金交付的直接证据为核心
- 质证重点：借条（书证）真实性、转账流水（电子数据）完整性、利息约定合法性
- 诉讼时效：一般民事案件 3 年，届满前是否有中断事由需重点关注
- 借贷关系认定：需同时证明借贷合意与资金交付\
"""


def format_parties_block(parties: list[dict]) -> str:
    """将当事人列表格式化为 prompt 输入块。
    Format the parties list into a readable prompt block.
