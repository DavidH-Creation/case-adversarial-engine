...
class MinutesGenerator:
    ...
    def generate(
        ...
        submitted_ev_ids = [
            ev.evidence_id for ev in final_idx.evidence
        ...
        for rec in xexam.records:
            ...
            for op in rec.opinions:
                xexam_lines.append(
                    f"  - [{op.dimension.value}] {op.verdict.value}: {op.reasoning}"
                )
        ...
        for iss in issue_tree.issues:
            ...
            issue_lines.append(
                f"- {iss.issue_id}: {iss.title}{cat_str} "
        ...
        for f in focus:
            focus_lines.append(
                f"- {f.evidence_id} / {f.issue_id} "
                f"[{f.dimension.value}]: {f.dispute_summary}"
            )
