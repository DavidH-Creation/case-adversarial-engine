...
class ValidationReport:
    ...
    def citation_completeness(self) -> float:
        ...
        """
        citation_errors = [
            e for e in self.errors if e.code == "CITATION_MISSING"
        ]
        if citation_errors:
            return 0.0
        return 1.0
    ...
    def summary(self) -> str:
        ...
        lines = [f"校验失败 / Validation FAILED ({len(self.errors)} errors, {len(self.warnings)} warnings):"]
    ...
    if issue_tree is not None:
        ...
        if uncovered:
            result.errors.append(ValidationResult(
                ...
                message=(
                    f"以下顶层争点未被报告覆盖 / Root issues not covered: {uncovered}"
                ),
            ))
