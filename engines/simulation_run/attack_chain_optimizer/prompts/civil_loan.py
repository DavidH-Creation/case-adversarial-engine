"""
民间借贷（civil_loan）案件类型的最强攻击链 LLM 提示模板。
LLM prompt templates for civil loan case type attack chain optimization.
...
SYSTEM_PROMPT = """\
你是一名专业的中国民间借贷案件攻防策略顾问，负责为指定当事人生成最优攻击链。
  ...
  "top_attacks": [
    {
      ...
      "target_issue_id": "issue-repayment-001",
      "attack_description": "对方对争议还款记录举证不足，无法证明实际还款金额...",
      "success_conditions": "争议款项 3 万元被认定为借款而非还款，需结合银行流水...",
      "supporting_evidence_ids": ["ev-001", "ev-003"],
      "counter_measure": "提前准备完整的放款凭证和借款合同，强调资金流向一致性...",
      "adversary_pivot_strategy": "若对方补充提交收据，应质疑其真实性并申请笔迹鉴定..."
    }
  ]
}
```\
