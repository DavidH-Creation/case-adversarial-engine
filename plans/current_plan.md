...
当前目标：`v1.5` ✅ 已完成（2026-03-29）

v1.5 在 v1.2 基础上完成决策链升级。争点排序从枚举优先级升级到加权 composite_score；裁判路径树增加 admissibility gate / fallback path / result scope；行动建议新增案型自适应 LLM 策略层（party-specific strategy）；执行摘要真正消费上游决策产物。
