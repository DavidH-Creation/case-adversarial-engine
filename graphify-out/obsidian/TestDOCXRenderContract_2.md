---
source_file: "C:\Users\david\dev\case-adversarial-engine\engines\report_generation\v3\tests\test_real_estate_acceptance.py"
type: "code"
community: "C: Users"
location: "L652"
tags:
  - graphify/code
  - graphify/INFERRED
  - community/C:_Users
---

# TestDOCXRenderContract

## Connections
- [[.test_real_estate_docx_passes_lint()]] - `method` [EXTRACTED]
- [[ConditionalNode]] - `uses` [INFERRED]
- [[ConditionalScenarioTree]] - `uses` [INFERRED]
- [[CoverSummary]] - `uses` [INFERRED]
- [[DOCX render contract subset must pass for real_estate.]] - `rationale_for` [EXTRACTED]
- [[EvidenceBasicCard]] - `uses` [INFERRED]
- [[EvidenceKeyCard]] - `uses` [INFERRED]
- [[EvidencePriority]] - `uses` [INFERRED]
- [[EvidencePriorityCard]] - `uses` [INFERRED]
- [[FactBaseEntry]] - `uses` [INFERRED]
- [[FourLayerReport]] - `uses` [INFERRED]
- [[IssueMapCard]] - `uses` [INFERRED]
- [[Layer1Cover]] - `uses` [INFERRED]
- [[Layer2Core]] - `uses` [INFERRED]
- [[Layer3Perspective]] - `uses` [INFERRED]
- [[Layer4Appendix]] - `uses` [INFERRED]
- [[LintResult]] - `uses` [INFERRED]
- [[LintSeverity]] - `uses` [INFERRED]
- [[PerspectiveOutput]] - `uses` [INFERRED]
- [[RenderContractViolation]] - `uses` [INFERRED]
- [[SectionTag]] - `uses` [INFERRED]
- [[TimelineEvent_1]] - `uses` [INFERRED]
- [[test_real_estate_acceptance.py]] - `contains` [EXTRACTED]

#graphify/code #graphify/INFERRED #community/C:_Users