---
source_file: "C:\Users\david\dev\case-adversarial-engine\engines\case_structuring\evidence_weight_scorer\schemas.py"
type: "code"
community: "C: Users"
location: "L22"
tags:
...
- [[LLM 输出的单条证据权重评分（中间模型，由规则层进一步校验）。]] - `rationale_for` [EXTRACTED]
...
- [[从四维评分 + 质疑记录派生可采性状态枚举。 规则（保守策略）： - 已被排除（evidence_status 含 ex]] - `uses` [INFERRED]
- [[低风险时 admissibility_notes 为 None 也正常更新。]] - `uses` [INFERRED]
- [[对证据索引中的所有证据进行四维权重评分。 Args inp 评分器输入 Returns]] - `uses` [INFERRED]
...
- [[规则层：校验 LLM 输出，返回 evidence_id → 已校验字段字典的映射。 过滤规则： 1. evidence_1]] - `uses` [INFERRED]
- [[计算证据稳定性得分 (0-1)。 稳定性 = 即使被质证也不容易崩的程度。 - authenticity_risk=lo]] - `uses` [INFERRED]
- [[证据权重评分器。 Args llm_client 符合 LLMClient 协议的客户端实例 case_]] - `uses` [INFERRED]
- [[评分后原有字段（title, summary 等）不受影响。]] - `uses` [INFERRED]
- [[调用 LLM（结构化输出优先，fallback 到 json_utils），失败时返回 None（不抛异常）。 Call LLM (struc]] - `uses` [INFERRED]
